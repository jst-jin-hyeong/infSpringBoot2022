...
const $listenIcon = document.querySelector(
  ...
  $listenIcon.addEventListener("click", (e) => {
    ...
    const $page = document.querySelector("#textContent");
    console.log($newDiv);
...
// 네비게이션에 있는 타임스탬프
function eventListenerTimeStamp() {
  const $listenTimeStamp = document.querySelector(
      `button.time-stamp`
    );
    $listenTimeStamp.addEventListener("click", (e) => {
      // 아이콘을 페이지에 추가하기 위해 createElement 하기
      const $newSpan = document.createElement("span");
      $newSpan.classList.add("time-label");
      $newSpan.innerHTML = `<span class="time-label2"></i>`;
